...
namespace DotNet8WebApi.ExceptionHandlingMiddleware.Models
{
    public class Result<T>
    {
        ...
        {
            return new Result<T> { Message = message, StatusCode = EnumStatusCode.OK, IsSuccess = true };
        }
        public static Result<T> Success(T data, string message = "Success.")
        {
            return new Result<T> { Message = message, Data = data, StatusCode = EnumStatusCode.OK, IsSuccess = true };
        }
        public static Result<T> Fail(string message = "Fail.")
        {
            return new Result<T> { Message = message, StatusCode = EnumStatusCode.BadRequest, IsSuccess = false };
        }
